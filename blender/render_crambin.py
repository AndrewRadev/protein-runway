...
class RenderCrambin(bpy.types.Operator):

    bl_idname = "Protein_Runway.Render_Crambin"
    bl_label = "Render Crambin"
    ...
    def execute(self, context):
        ...
        u = mda.Universe("C:/Users/mkrun/Downloads/crambin_md.pdb")
        atoms = u.atoms

        for atom in atoms:
            x = self.create_atom(atom.names, gray, 1.0)

            self.set_atom_position(x, atom.positions, frame=1)
    ...
    def create_atom(slef, name, color, radius):
...
def draw_create_render_menu(self, context):
    self.layout.operator(RenderCrambin.bl_idname, text="Render Crambin")


def register():
    bpy.utils.register_class(RenderCrambin)


def unregister():
    bpy.utils.unregister_class(RenderCrambin)
